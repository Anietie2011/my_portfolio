      ...
      <div className="mb-20">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {skills.map((skill, index) => (
            <div
              key={index}
              className="bg-white/5 border border-white/10 rounded-3xl p-6 text-center hover:bg-white/10 transition"
            >
              {skill}
            </div>
          ))}
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
          {learningSkills.map((skill, index) => (
            <div
              key={index}
              className="bg-gradient-to-br from-cyan-500/10 to-purple-500/10 border border-white/10 rounded-3xl p-6 text-center hover:scale-105 transition"
            >
              {skill}
            </div>
          ))}
